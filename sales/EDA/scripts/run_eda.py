...
def main():
    ...
    run = neptune.init_run(tags=["eda", "walmart-sales"])
    ...
    run["data/statistics/diffSales_warm_vs_cold"] = "$ {:,}".format(
        round(sales_warm_vs_cold)
    )
    ...
    plt.figure()
    sales_vs_isholiday = sns.lineplot(
        x="Month", y="Weekly_Sales", hue="IsHoliday", data=df
    )
    ...
    plt.figure()
    sales_vs_temp = sns.histplot(
        data=df.groupby("Weekly_Sales")["Temperature"].mean(), bins=30
    )
    ...
    run["data/visualizations/sales_vs_month"].upload(
        File.as_image(sales_vs_month.figure)
    )
    run["data/visualizations/sales_vs_quarter"].upload(
        File.as_image(sales_vs_qt.figure)
    )
    run["data/visualizations/sales_vs_store"].upload(
        File.as_image(sales_vs_store.figure)
    )
    run["data/visualizations/sales_vs_dept"].upload(File.as_image(sales_vs_dept.figure))
    run["data/visualizations/sales_vs_IsHoliday"].upload(
        File.as_image(sales_vs_isholiday.figure)
    )
    run["data/visualizations/sales_vs_temperature"].upload(
        File.as_image(sales_vs_temp.figure)
    )
    run["data/visualizations/sales_vs_storeType"].upload(
        File.as_image(sales_vs_storeType.figure)
    )
