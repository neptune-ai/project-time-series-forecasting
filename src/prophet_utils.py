...
def log_forecast_plots(run, model, forecast, log_interactive=True):
    if log_interactive:
        ...
        run['visualizations/forecast'] = File.as_html(fig1)
        run['visualizations/forecast_components'] = File.as_html(fig2)
    else:
        fig1 = model.plot(forecast)
        fig2 = model.plot_components(forecast)
        run['visualizations/forecast'].upload(fig1)
        run['visualizations/forecast_components'].upload(fig2)
...
def log_plot_cross_validation_metric(run, cv, metric='mse', log_interactive=True):
    ...
    if log_interactive:
        run['visualizations/cross_validation_metric'] = File.as_html(fig)
    else:
        run['visualizations/cross_validation_metric'].upload(fig)


def log_prophet_metadata(run, model, forecast=None, cv=None, metric='mse', log_interactive=True):
    log_config(run, model)
    log_params(run, model)
